...
Vector3 headPos;
Vector3 initKinectPos;
bool kinectFound=false;
public float moveGain = 0.1f;
...
void Update()
{
    ...
    //Finds head position
        if (GameObject.Find("Head") == null)
        {
            ...
            transform.eulerAngles = initPos.eulerAngles;
            return;
        }
            if(!kinectFound)
            {
                initKinectPos = GameObject.Find("Head").transform.localPosition;
                kinectFound=true;
            }
            
            //Probably need to involve damping
            headPos = GameObject.Find("Head").transform.localPosition;
            transform.localPosition = new Vector3(
                (headPos.x - initKinectPos.x) * moveGain, 
                ((headPos.y - initKinectPos.y) * moveGain)+1.5f, 
                transform.localPosition.z
                );
            Debug.Log(headPos.x + ", " + initKinectPos.x);
